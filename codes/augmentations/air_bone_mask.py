...
def air_bone_mask(img, **kwargs):
        ...
        upper = ((tissue_window[1]) - (x_min))/(x_max-(x_min))
        lower = ((tissue_window[0]) - (x_min))/(x_max-(x_min))
        tissue = base.hu_clip(img, (lower, upper), None, True)        
        
        return np.stack((air, bone, tissue))
